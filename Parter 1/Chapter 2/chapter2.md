...
オブジェクト指向システムは、**協調するオブジェクトの網（web）**です。システムは、オブジェクトを作成し、互いにメッセージを送れるように**接続**して構築します。システムのふるまいは、オブジェクトの**合成（コンポジション）**――どのオブジェクトを選び、どう接続するか――の結果として**創発的**に現れます（Figure 2.1）。
...
## Values and Objects
システムを設計する際は、変化しない量や測定値を表す**値（value）**と、**同一性**を持ち時間とともに状態が変化し得て計算過程をモデル化する**オブジェクト**を区別することが重要です。私たちの多くが用いるオブジェクト指向言語では、両者が同じ言語構造（クラス）で実装されるため混同が起こります。

**値（value）**は、固定の量を表す**不変（immutable）なインスタンス**です。個別の同一性を持たないため、状態が同じであれば2つの値インスタンスは事実上同一です。したがって、2つの値の**同一性**を比較することには意味がなく、そのせいで微妙なバグを招くこともあります――たとえば、2つの new Integer(999) の比較方法の違いを思い出してください。これが、Java で string1.equals(string2) を使い、string1 == string2 を使わないよう教えられる理由です。

一方で**オブジェクト**は、振る舞いを時間とともにモデル化するために**可変の状態**を使用します。将来、異なるメッセージを受け取れば状態が分岐し得るため、たとえ現在の状態がまったく同じでも、同じ型の2つのオブジェクトは別々の同一性を持ちます。

実務では、システムを2つの「世界」に分けて考えます。すなわち、**関数的に扱う値**と、システムの**状態を伴う振る舞いを実装するオブジェクト**です。Part III では、どちらの世界で作業しているかによって、私たちのコーディングスタイルがどのように変化するかを示します。

本書では、**object** という語を、同一性・状態・処理を備えたインスタンスだけを指す用語として用い、**値（value）**は含めません。一般に受け入れられ、かつ他の意味で過負荷になっていない（entity や process のように）別の適切な用語は見当たらないためです。
